...
function loadAlerts(element) {
    $.ajax('/alert/getalerts', {
        ...
        success:function(data) {
            ...
            for (var i = 0; i < len; i++) {
                outputHTML +=
                    '<div id="' + i + '" style="background-color: #FF6666; border-radius: 5px; margin: 10px; padding: 10px">' +
                    '<div class="row" >' +
                    '<div class="col-lg-8">'+
                    ...
                    '<button type="button" class="btn btn-primary btn-md" style="float:left; margin: 0px 5px 5px 0px;" onclick="resolvealert(' + i + ',' + data[i].alertID + ')">' + 'Health Conditions' + '</button>' +
